...
export default function EditMenuItemPage() {
  ...
  const loadMenuItem = async () => {
    try {
      setLoading(true);
      // Получаем все пункты меню и находим нужный
      ...
      if (result.success) {
        ...
        if (item) {
          ...
          showAlert({ type: 'error', title: 'Пункт меню не найден' });
          router.push(`/admin/menus/${menuId}`);
        }
      }
    } catch (error) {
      console.error('Error loading menu item:', error);
      showAlert({ type: 'error', title: 'Ошибка при загрузке пункта меню' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (result.success) {
        showAlert({ type: 'success', title: 'Пункт меню обновлен' });
        router.push(`/admin/menus/${menuId}`);
      } else {
  ...
  const rootItems = menuItems.filter(item => !item.parentId && item.id !== itemId);

  return (
    <div className="grid gap-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[#00205B]">Редактировать пункт меню</h1>
