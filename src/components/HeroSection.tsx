...
const HeroSection = () => {
  ...
  const getCityName = () => {
    ...
    if (regionalData?.officeName) {
      // Если officeName уже в нужном падеже (например "Крыму"), используем его напрямую
      // Проверяем, есть ли предлог "в" в начале - если да, значит уже в правильном падеже
      const officeName = regionalData.officeName.trim();
      
      // Маппинг для склонения названий городов в предложный падеж (только если это название города)
      const cityName = officeName.toLowerCase();
      const declensionMap: Record<string, string> = {
        ...
        'партенит': 'Партените',
        'кача': 'Каче',
        'крыму': 'Крыму', // Добавляем поддержку "Крыму"
        'крым': 'Крыму',
      };
      ...
      // Если склонение не найдено, возвращаем оригинальное название (может быть уже в правильном падеже)
      return officeName;
    }
