...
export async function getRegionalContacts(region: string): Promise<RegionalContactData> {
  ...
  try {
    ...
    // Используем 'no-store' для получения актуальных данных (кэш управляется на сервере через revalidateTag)
    const response = await fetch(`${baseUrl}/api/regions`, {
      cache: 'no-store', // Не кэшируем на клиенте, чтобы получать актуальные данные
    });
